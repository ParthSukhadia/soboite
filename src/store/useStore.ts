...
import { FALLBACK_RESTAURANTS, FALLBACK_DISHES } from '../lib/mockData';
...
const LOCAL_STORAGE_KEY = 'soboite_local_restaurants_v1';

const loadLocalRestaurants = (): Restaurant[] => {
  try {
    ...
    console.warn('Failed to read local restaurants', e);
    return [];
  }
};

const saveLocalRestaurants = (items: Restaurant[]) => {
  try {
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(items));
  } catch (e) {
    console.warn('Failed to save local restaurants', e);
  }
};
  ...
  (set, get) => ({
    ...
    fetchData: async () => {
      ...
      try {
        ...
        // If Supabase timed out or returned nothing, fall back to local mock data so UI remains usable.
        const shouldUseFallback = !!restRes.error || !!dishRes.error || (mappedRests.length === 0 && mappedDishes.length === 0);

        // Merge any locally-saved restaurants (from previous failed adds) so they appear in the UI.
        const local = loadLocalRestaurants();
        const mergeRestaurants = (base: Restaurant[]) => {
          const byId = new Map<string, Restaurant>();
          [...base, ...local].forEach((r) => {
        ...
        if (shouldUseFallback) {
          console.warn('Supabase returned no data or errored — using fallback mock data');
          set({
            restaurants: mergeRestaurants(FALLBACK_RESTAURANTS),
            dishes: FALLBACK_DISHES,
            restaurantTypes: uniqueSorted(derivedTypes.length > 0 ? derivedTypes : FALLBACK_RESTAURANTS.map(r => r.type).filter(Boolean) as string[]),
            cuisines: uniqueSorted(derivedCuisines.length > 0 ? derivedCuisines : FALLBACK_RESTAURANTS.map(r => r.cuisine).filter(Boolean) as string[]),
            flavorTags: uniqueSorted(derivedFlavorTags.length > 0 ? derivedFlavorTags : [])
          });
        } else {
          set({
            restaurants: mergeRestaurants(mappedRests),
            dishes: mappedDishes,
    ...
    addRestaurant: async (restaurant) => {
      ...
      if (error) {
        // Supabase insert failed — save locally so UI reflects the new restaurant.
        console.warn('Supabase insert failed, saving restaurant locally:', error);
        try {
          const existing = loadLocalRestaurants();
          const next = [...existing.filter(r => r.id !== normalizedRestaurant.id), normalizedRestaurant];
          saveLocalRestaurants(next);
          set((state) => ({ restaurants: [...state.restaurants, normalizedRestaurant] }));
          return;
        } catch (e) {
          throw new Error(typeof error === 'object' && (error as any).message ? (error as any).message : String(error));
        }
      }
      set((state) => ({ restaurants: [...state.restaurants, normalizedRestaurant] }));
    },
